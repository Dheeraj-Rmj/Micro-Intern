...
export default function OnboardingPage() {
  ...
  const captureFace = async () => {
    if (videoRef.current) {
      ...
      if (detection) {
        ...
        alert(
          "No face detected. Please ensure you are in a well-lit area and looking directly at the camera.",
        );
      }
    }
  };
  ...
  if (loading)
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FDFCF9]">Loading...</div>
    );
  if (error)
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FDFCF9] text-red-600">
        {error}
      </div>
    );
      ...
      <div className="w-full max-w-3xl bg-white rounded-[32px] shadow-sm border border-black/5 p-8">
        {step === 1 && (
          <div className="space-y-6 animate-in fade-in">
            ...
            <h2 className="text-xl font-semibold flex items-center gap-2 pt-6 border-t border-black/5">
              <User className="w-5 h-5" /> Admin Details
            </h2>
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider mb-2">
            ...
            <div className="flex justify-end pt-4">
              <button
                onClick={handleNext}
                className="px-6 py-3 bg-[#111111] text-white rounded-xl text-sm font-semibold flex items-center gap-2 hover:scale-105 transition-transform"
              >
        ...
        {step === 2 && (
          <div className="space-y-6 animate-in fade-in">
            ...
            <div className="flex justify-between pt-4">
              <button
                onClick={handleBack}
                className="px-6 py-3 bg-black/5 rounded-xl text-sm font-semibold"
              >
                Back
              </button>
              <button
                onClick={handleNext}
                disabled={!logoUrl}
                className="px-6 py-3 bg-[#111111] text-white rounded-xl text-sm font-semibold disabled:opacity-50"
              >
                Next
              </button>
            </div>
        ...
        {step === 3 && (
          <div className="space-y-6 animate-in fade-in">
            ...
            <p className="text-sm text-black/60">
              Please upload a valid Government ID and perform a 3D Face Liveness check.
            </p>
                ...
                {govDocUrls.length > 0 ? (
                  ...
                  <>
                    ...
                    <span className="text-xs font-medium text-black/60">
                      Upload Passport or Driver&apos;s License (Max 5MB)
                    </span>
                  </>
                )}
                <input
                  ...
                  accept="image/*"
                  className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
                  onChange={(e) => handleImageUpload(e, "govDoc")}
                />
              </div>
            </div>
              ...
              {faceScanData ? (
                ...
                <div className="flex flex-col items-center gap-4">
                  <video
                    ...
                    autoPlay
                    muted
                    className="w-full max-w-sm rounded-2xl bg-black/5 h-[280px] object-cover"
                  />
                  <div className="flex gap-3">
                    <button
                      onClick={startCamera}
                      className="px-4 py-2 bg-black/5 rounded-lg text-sm font-semibold flex items-center gap-2"
                    >
                      <Camera className="w-4 h-4" /> Start Camera
                    </button>
                    <button
                      onClick={captureFace}
                      className="px-4 py-2 bg-[#111111] text-white rounded-lg text-sm font-semibold"
                    >
                      Verify Liveness
                    </button>
                  </div>
            ...
            <div className="flex justify-between pt-4">
              ...
              <button
                onClick={handleNext}
                disabled={!govDocUrls.length || !faceScanData}
                className="px-6 py-3 bg-[#111111] text-white rounded-xl text-sm font-semibold disabled:opacity-50"
              >
                Next
              </button>
        ...
        {step === 4 && (
          <div className="space-y-6 animate-in fade-in">
            ...
            <p className="text-sm text-black/60">
              Draw your signature below to authorize the Memorandum of Understanding (MoU).
            </p>
            ...
            <div className="flex justify-between pt-4">
              ...
              <button
                ...
                disabled={submitting}
                className="px-6 py-3 bg-indigo-600 text-white rounded-xl text-sm font-semibold flex items-center gap-2"
              >
        ...
        {step === 5 && (
          <div className="py-12 flex flex-col items-center text-center animate-in zoom-in duration-300">
            ...
            <p className="text-black/60 max-w-md">
              Your company details and eKYC documents have been securely transmitted to the Super
              Admin for final verification. Once approved, you will receive an email with your
              signed MoU.
            </p>
          </div>
        )}
