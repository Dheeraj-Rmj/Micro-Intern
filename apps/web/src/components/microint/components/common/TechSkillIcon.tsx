...
export const TechSkillIcon: React.FC<TechSkillIconProps> = ({
  ...
  if (!iconUrl || imgError) {
    ...
    if (
      ...
      return (
        <Database className={`text-blue-400 ${className}`} style={{ width: size, height: size }} />
      );
    }
    if (
      ...
      return (
        <Cloud className={`text-amber-400 ${className}`} style={{ width: size, height: size }} />
      );
    }
    if (normalizedKey.includes("system") || normalizedKey.includes("architect")) {
      return (
        <Layers className={`text-emerald-400 ${className}`} style={{ width: size, height: size }} />
      );
    }
