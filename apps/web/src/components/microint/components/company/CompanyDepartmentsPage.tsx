    ...
    <div className="space-y-8 animate-in fade-in duration-300 pb-16">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-2 border-b border-black/5 dark:border-white/10">
        ...
        <div className="flex items-center gap-3 shrink-0">
          <button 
            onClick={() => showToast('Sync Complete', 'Department data synchronized with Active Directory.', 'success')}
          ...
          <button
            onClick={() => showToast('Create Department', 'Department creation overlay triggered.', 'info')}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {loading ? (
          ...
          <div key={i} className="p-6 rounded-[32px] bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/10 shadow-sm relative overflow-hidden group hover:border-black/20 dark:hover:border-white/20 transition-all">
            ...
            <div className="flex gap-4 text-xs font-mono text-black/50 dark:text-white/50">
              <div className="flex flex-col">
                <span className="mb-1 uppercase tracking-widest text-[9px] text-black/40 dark:text-white/40">Headcount</span>
                <span className="text-black dark:text-white font-bold">{dept.headcount} Seats</span>
              </div>
              <div className="flex flex-col">
                <span className="mb-1 uppercase tracking-widest text-[9px] text-black/40 dark:text-white/40">Budget</span>
                <span className="text-emerald-600 dark:text-emerald-400 font-bold">{typeof dept.budget === 'number' ? `$${dept.budget}` : dept.budget}</span>
              </div>
            </div>
          </div>
        ))}
