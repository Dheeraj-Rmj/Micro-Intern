...
export const AchievementsPage: React.FC = () => {
  ...
  return (
    <div className="pb-12 text-black dark:text-white max-w-[1200px] mx-auto w-full font-sans">
      ...
      {/* Trust Score Banner */}
      <div className="mb-12 p-8 sm:p-10 rounded-[40px] bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/10 shadow-sm relative overflow-hidden">
        <div className="absolute -top-24 -right-24 w-64 h-64 bg-white/15 dark:bg-white/10 rounded-full blur-[80px] pointer-events-none" />
        <div className="absolute -bottom-24 -left-24 w-64 h-64 bg-white/15 dark:bg-white/10 rounded-full blur-[80px] pointer-events-none" />

        <div className="flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
          <div>
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-black/5 dark:bg-white/5 text-[10px] font-bold tracking-widest uppercase mb-4 text-black/60 dark:text-white/70">
      ...
      {/* Badges Section */}
      <div className="mb-12">
        <h2 className="text-2xl font-serif tracking-tight mb-6 flex items-center gap-3 text-black dark:text-white border-b border-black/5 dark:border-white/10 pb-4">
        ...
        {achievements.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {achievements.map((b) => (
              <div
                key={b.id}
                className="p-6 rounded-[32px] bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/10 shadow-sm hover:shadow-lg transition-all flex flex-col justify-between group"
              >
                <div>
                  <div className="flex items-center justify-between mb-6">
                    <div className="w-12 h-12 rounded-full bg-black/5 dark:bg-white/5 flex items-center justify-center text-black dark:text-white group-hover:scale-110 transition-transform">
                      <Sparkles className="w-5 h-5" />
                    </div>
                    <span className="text-[10px] font-bold px-3 py-1 rounded-full bg-[#111111] dark:bg-white text-white dark:text-black uppercase tracking-widest shadow-sm">
                      {b.level}
                    </span>
                  </div>

                  <h3 className="text-lg font-serif tracking-tight text-black dark:text-white mb-2 leading-tight">
                    {b.title}
                  </h3>
                  <p className="text-xs text-black/60 dark:text-white/70 font-medium leading-relaxed mb-6">
                    {b.description}
                  </p>
                </div>

                <div className="pt-4 border-t border-black/5 dark:border-white/10 flex flex-col gap-2">
                  <span className="text-[10px] font-bold text-black/40 dark:text-white/50 uppercase tracking-widest">
                    {b.category}
                  </span>
        ...
        <div className="grid gap-4">
          {completedTrials.length > 0 ? (
            completedTrials.map((t) => (
              <div
                key={t.id}
                className="p-6 md:p-8 rounded-[40px] bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/10 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-6 hover:shadow-lg transition-all"
              >
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <h3 className="text-2xl font-serif tracking-tight text-black dark:text-white leading-tight mt-1">
                      {t.title}
                    </h3>
                    <p className="text-xs font-bold text-black dark:text-white uppercase tracking-widest mt-2 flex items-center gap-1.5">
                ...
                <button
                  ...
                  className="w-full sm:w-auto px-8 py-3.5 rounded-full bg-[#111111] dark:bg-white text-white dark:text-black font-bold text-sm shadow-sm transition-transform hover:scale-105 cursor-pointer flex items-center justify-center gap-2"
                >
            ...
            <div className="p-12 rounded-[40px] bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/10 text-center shadow-sm">
