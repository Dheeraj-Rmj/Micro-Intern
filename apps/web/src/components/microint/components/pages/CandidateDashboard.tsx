...
export const CandidateDashboard: React.FC = () => {
  ...
  const [activeModal, setActiveModal] = useState<'none' | 'calendar' | 'add-widget' | 'widget-detail'>('none');
          ...
          if (ghRes && ghRes.ok) {
            ...
            (ghData.items || []).forEach((repo: any, idx: number) => {
              results.push({
                ...
                url: repo.html_url,
                content: repo.description || 'Official open-source repository and codebase documentation.',
                score: 0.96 - idx * 0.03,
              });
            });
          }
  ...
  const activeTrials = trials.filter((t) => t.status === 'open' || t.status === 'in_progress' || t.status === 'applied');
  const displaySkills =
  ...
  const last7DaysLabels = Array(7).fill('').map((_, i) => dayNames[(todayIndex - 6 + i + 7) % 7]);
  ...
  const appRateVal = applications.length > 0
  ...
  const matchScore1 = trials[0] ? ((trials[0] as any).matchScore || averageMatchScore || 75) : averageMatchScore || 75;
  const matchScore2 = trials[1] ? ((trials[1] as any).matchScore || averageMatchScore || 85) : averageMatchScore || 85;
  const matchScore3 = trials[2] ? ((trials[2] as any).matchScore || averageMatchScore || 92) : averageMatchScore || 92;
  ...
  const getCalendarDays = () => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      const matchedTrial = activeTrials.find((_, idx) => ((idx * 7 + 14) % daysInMonth || 15) === d);
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            ...
            <div className="mt-auto h-28 relative flex items-center justify-center">
              <svg viewBox="0 0 100 40" className="w-full h-full overflow-visible preserve-3d">
                ...
                <g transform={`translate(66, ${Math.max(1, appY - 8)})`} className="transition-all duration-700">
                  <rect width="18" height="6.5" rx="3.25" className="fill-[#111111] dark:fill-[#E1E0CC]" />
                  <text x="9" y="4.6" className="fill-white dark:fill-black" fontSize="3.8" fontWeight="bold" textAnchor="middle">
                    +{appRateVal}%
                  </text>
                </g>
              </svg>
        ...
        {enabledWidgets['tavily-search'] && (
          ...
          >
            ...
            <div className="relative z-10 flex flex-wrap items-center gap-2 text-xs">
              <span className="text-[10px] font-mono uppercase text-black/40 dark:text-white/40 mr-1">Try:</span>
              {[
                { label: 'Next.js 15 features', query: 'Next.js 15 server actions and react compiler' },
                { label: 'Microservices vs Monolith', query: 'System design microservices vs monolith tradeoffs' },
              ...
              <div className="space-y-2 max-h-36 overflow-y-auto pr-1">
                {activeTrials.length > 0 ? (
                  activeTrials.map((trial, index) => (
                    <div key={trial.id} className="flex items-center justify-between p-3 rounded-2xl bg-black/5 dark:bg-white/5 text-xs">
              ...
              <div>
                ...
                <div className="space-y-3 mb-6">
                  <div className="p-3.5 rounded-2xl bg-black/5 dark:bg-white/5 flex items-center justify-between text-xs font-semibold">
                    <span className="truncate pr-2">{trials[0] ? trials[0].title : 'Frontend Engineering (React / Next.js)'}</span>
                    <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-500 shrink-0">{matchScore1}% Match</span>
                  </div>
                  <div className="p-3.5 rounded-2xl bg-black/5 dark:bg-white/5 flex items-center justify-between text-xs font-semibold">
                    <span className="truncate pr-2">{trials[1] ? trials[1].title : 'AI Engineering & LLM Integrations'}</span>
                    <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-500 shrink-0">{matchScore2}% Match</span>
                  </div>
                  <div className="p-3.5 rounded-2xl bg-black/5 dark:bg-white/5 flex items-center justify-between text-xs font-semibold">
                    <span className="truncate pr-2">{trials[2] ? trials[2].title : 'Full-Stack Architecture & TypeScript'}</span>
                    <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-500 shrink-0">{matchScore3}% Match</span>
                  </div>
                </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-3 mb-6 max-h-60 overflow-y-auto pr-1">
                  {displaySkills.map((skill, index) => (
                    <div key={index} className="p-4 rounded-2xl bg-black/5 dark:bg-white/5 flex items-center gap-4">
            ...
            {selectedWidgetId === 'jina-reader' && (
              <div>
                ...
                <div className="flex flex-wrap items-center gap-2 mb-6">
                  <span className="text-xs font-mono uppercase text-black/40 dark:text-white/40 mr-1">Sample URLs:</span>
                  {[
                    { label: 'React 19 Repo', url: 'https://github.com/facebook/react' },
                    { label: 'React Documentation', url: 'https://react.dev' },
                    { label: 'ZenQuotes Random API', url: 'https://zenquotes.io/api/quotes/random' },
                  ].map((sample) => (
                    <button
                      key={sample.url}
                      onClick={() => {
                ...
                {jinaLoading ? (
                  <div className="p-12 rounded-3xl bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/10 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-xs text-black/50 dark:text-white/50 max-w-md">
                      Extracting page content, removing boilerplate, and converting to clean LLM-friendly Markdown text.
                    </p>
                  </div>
                ) : jinaError ? (
                  ...
                  <div className="p-12 rounded-3xl bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/10 text-center">
                    ...
                    <p className="text-xs text-black/50 dark:text-white/50 max-w-sm mx-auto">
                      Enter any web page, documentation, or repository URL above to test AI Web Reader live.
                    </p>
                  </div>
                )}
              </div>
            ...
            {selectedWidgetId === 'tavily-search' && (
              <div>
                ...
                <div className="flex flex-wrap items-center gap-1.5 mb-5">
                  ...
                  {[
                    { label: 'Next.js 15 Server Actions', query: 'Next.js 15 server actions best practices and react compiler' },
                    { label: 'Kafka vs RabbitMQ Architecture', query: 'Kafka vs RabbitMQ architecture differences system design' },
                    { label: 'React 19 Hooks Guide', query: 'What is new in React 19 hooks and useActionState' },
                  ].map((sample) => (
                    <button
                      key={sample.query}
                      onClick={() => {
                ...
                {tavilyLoading ? (
                  ...
                  <div className="py-10 px-6 rounded-2xl bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/10 text-center">
                    ...
                    <p className="text-xs text-black/50 dark:text-white/50 max-w-sm mx-auto">
                      Enter any technical question, trial requirement, or architecture pattern to search via MicroIntern AI.
                    </p>
                  </div>
                )}
              </div>
