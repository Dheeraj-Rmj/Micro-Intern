    ...
    <header className="fixed top-6 left-0 right-0 z-50 flex justify-center px-4 w-full">
      ...
      {mobileMenuOpen && (
        <div className="absolute top-24 left-4 right-4 rounded-3xl border border-slate-200 dark:border-white/10 bg-white/95 dark:bg-[#101010]/95 backdrop-blur-xl px-4 py-6 shadow-2xl space-y-4 md:hidden">
          <button onClick={() => scrollToSection('hero')} className="block w-full text-left py-2 px-4 rounded-xl font-medium text-slate-700 hover:bg-slate-50">
            Home
          </button>
          ...
          <button onClick={() => scrollToSection('faq')} className="block w-full text-left py-2 px-4 rounded-xl font-medium text-slate-700 hover:bg-slate-50">
            FAQ
          </button>
