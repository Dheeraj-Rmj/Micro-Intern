...
export const SignUpPage: React.FC = () => {
  ...
  const getStrengthLabel = (score: number) => {
    if (score === 0) return { label: 'Weak', color: 'bg-rose-500', width: 'w-1/4' };
    if (score === 1 || score === 2) return { label: 'Fair', color: 'bg-amber-500', width: 'w-2/4' };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (Object.keys(newErrors).length > 0) {
      setErrors(newErrors);
      showToast('Validation Error', 'Please check the form fields.', 'warning');
      return;
    }
      ...
      <div className="max-w-md mx-auto w-full">
        ...
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-xs sm:text-sm text-slate-600 dark:text-slate-400">
            Start taking real 2-5 day skill trials and unlock paid tech internships.
          </p>
        </div>
            ...
            <div className="pt-2">
              <label className="flex items-start gap-2.5 cursor-pointer">
                <input
                  ...
                  className="mt-0.5 rounded text-blue-600 focus:ring-blue-500 dark:bg-slate-800"
                />
