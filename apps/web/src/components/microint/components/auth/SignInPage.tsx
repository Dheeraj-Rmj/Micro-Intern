...
export const SignInPage: React.FC<SignInPageProps> = ({ initialPortal = 'candidate' }) => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.ctrlKey || e.metaKey) && e.shiftKey) {
        ...
        if (key === 'R') {
          ...
          showToast('Private Shortcut Unlocked', 'CTRL+SHIFT+R: Recruiter ATS Login Modal opened.', 'info');
        } else if (key === 'A') {
          e.preventDefault();
          setActiveModal('company');
          showToast('Private Shortcut Unlocked', 'CTRL+SHIFT+A: Company Admin Governance Modal opened.', 'info');
        } else if (key === 'S') {
          e.preventDefault();
          setActiveModal('superadmin');
          showToast('Private Shortcut Unlocked', 'CTRL+SHIFT+S: System-Ops Super Admin Modal opened.', 'info');
        }
      }
    };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="mb-6">
          ...
          <p className="text-xs opacity-65 font-mono">
            ...
            {initialPortal === 'enterprise' && 'enterprise.microintern.com • Shared Enterprise Access'}
            {initialPortal === 'ops' && 'ops.microintern.com • Super Admin Governance (/system-ops)'}
          </p>
        </div>
        ...
        {initialPortal === 'enterprise' && (
          <div className="mb-5 p-3 rounded-2xl bg-purple-500/10 border border-purple-500/30 text-xs">
            ...
            <p className="opacity-80 text-[11px] leading-relaxed">
              Company Admins and Recruiters share this login page. The backend detects your role automatically. Recruiters cannot self-register.
            </p>
          </div>
        )}
        ...
        {initialPortal === 'ops' && (
          <div className="mb-5 p-3 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-xs">
            ...
            <p className="opacity-80 text-[11px] leading-relaxed">
              This route is private. 5-stage middleware verifies Auth, Role, Tenant, MFA, and Device Trust. Unauthorized attempts return 403 Forbidden.
            </p>
          </div>
        )}
        ...
        >
          ...
          >
            ...
            <p className="text-xs opacity-70 mb-5 leading-relaxed font-mono">
              {activeModal === 'recruiter' && 'enterprise.microintern.com • Recruiters cannot self-register.'}
              {activeModal === 'company' && 'enterprise.microintern.com • Organization eKYC Governance.'}
      ...
      {showPrivacyModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in" onClick={() => setShowPrivacyModal(false)}>
          ...
          >
            ...
            <div className="overflow-y-auto px-7 py-5 text-sm leading-relaxed space-y-4">
              <p className="text-xs font-semibold uppercase tracking-wider opacity-60">Last updated: August 2025</p>
              {[
                { title: '1. Data We Collect', body: 'We collect information you provide directly (name, email, resume, profile details) and usage data generated as you interact with MicroIntern (pages visited, trials started, submissions made).' },
                { title: '2. How We Use Your Data', body: 'Your data is used to operate and improve MicroIntern, match you with relevant trials, send important platform notifications, and generate anonymised analytics. We do not sell your personal data.' },
                { title: '3. Resume & Profile Data', body: 'Uploaded resumes and profile information may be shared with companies whose trials you apply to. You control which companies can see your profile through your privacy settings.' },
                { title: '4. Cookies', body: 'MicroIntern uses essential cookies to keep you logged in and functional cookies to remember your preferences. No third-party advertising cookies are used.' },
                { title: '5. Data Retention', body: 'Your account data is retained as long as your account is active. You may request deletion of your account and associated data at any time via Settings.' },
                { title: '6. Security', body: 'We implement industry-standard security measures including encryption in transit (TLS) and at rest. However, no system is 100% secure — please use a strong, unique password.' },
                { title: '7. Your Rights', body: 'You have the right to access, correct, export, or delete your data. Contact us at privacy@microintern.io for any data-related requests.' },
              ].map((s) => (
                <div key={s.title}>
      ...
      {showAgreementModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in" onClick={() => setShowAgreementModal(false)}>
          <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" />
          <div
            className={`relative rounded-[32px] shadow-2xl w-full max-w-lg max-h-[80vh] flex flex-col overflow-hidden border ${
              darkMode ? 'bg-[#1F1F1F] text-white border-white/10' : 'bg-white text-black border-black/10'
          ...
          >
            ...
            <div className="overflow-y-auto px-7 py-5 text-sm leading-relaxed space-y-4">
              <p className="text-xs font-semibold uppercase tracking-wider opacity-60">Effective Date: August 2025</p>
              {[
                { title: '1. Platform Purpose & Eligibility', body: 'MicroIntern provides verified skill assessments, AI-validated certifications, and micro-internship trials for developers and engineers. Users must provide accurate profile credentials.' },
                { title: '2. Authenticity & Original Work', body: 'All submissions to trials and skill showcases must represent your own authentic work. Using automated bots without disclosure or plagiarising peer submissions will result in immediate badge revocation.' },
                { title: '3. AI Credential Verification', body: 'Our automated evaluation system inspects repositories, code quality, and technical completeness to assign verified Trust Scores. Scores are dynamically recalculated as you complete trials.' },
                { title: '4. Professional Conduct', body: 'Users in the Professional Network and Direct Messaging must maintain courteous, respectful, and professional communication with peers and hiring partners.' },
                { title: '5. Intellectual Property', body: 'You retain full ownership of code submitted to open community trials unless explicitly governed by a sponsored company agreement agreed upon before submission.' },
                { title: '6. Limitation of Liability', body: 'MicroIntern is provided "as is" without warranty of continuous uptime. We are not liable for employment outcomes between candidates and hiring companies.' },
              ].map((s) => (
                <div key={s.title}>
                  <h3 className="font-bold mb-1">{s.title}</h3>
                  <p className="opacity-80">{s.body}</p>
