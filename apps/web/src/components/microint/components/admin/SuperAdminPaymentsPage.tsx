...
import { CreditCard, RefreshCw, DollarSign, Activity, CheckCircle2, XCircle, Clock, Loader2 } from 'lucide-react';
    ...
    <div className="space-y-8 animate-in fade-in duration-300 pb-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="p-6 rounded-[32px] bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/10 shadow-sm relative overflow-hidden group col-span-2">
          ...
          <span className="text-xs font-mono uppercase text-black/50 dark:text-white/50 mb-1 block">Monthly Transaction Volume</span>
          {loading ? <Loader2 className="w-6 h-6 animate-spin text-black/20 dark:text-white/20 mt-2" /> : (
            <span className="text-4xl font-serif text-black dark:text-white">${(metrics?.monthlyVolume ?? 0).toLocaleString()}</span>
          )}
        </div>
        ...
        <div className="p-6 rounded-[32px] bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/10 shadow-sm relative overflow-hidden group">
          ...
          <span className="text-xs font-mono uppercase text-black/50 dark:text-white/50 mb-1 block">Successful TX</span>
          {loading ? <Loader2 className="w-6 h-6 animate-spin text-black/20 dark:text-white/20 mt-2" /> : (
            <span className="text-3xl font-serif text-black dark:text-white">{metrics?.successfulTransactions?.toLocaleString() ?? 0}</span>
          )}
        </div>
        ...
        <div className="p-6 rounded-[32px] bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/10 shadow-sm relative overflow-hidden group">
          ...
          <span className="text-xs font-mono uppercase text-black/50 dark:text-white/50 mb-1 block">Failed TX</span>
          {loading ? <Loader2 className="w-6 h-6 animate-spin text-black/20 dark:text-white/20 mt-2" /> : (
            <span className="text-3xl font-serif text-black dark:text-white">{metrics?.failedTransactions?.toLocaleString() ?? 0}</span>
          )}
        </div>
      </div>

      <div className="mt-8">
        <h3 className="text-lg font-serif text-black dark:text-white mb-4">Recent Stripe Payouts</h3>
          ...
          {loading ? (
            ...
            <div className="divide-y divide-black/5 dark:divide-white/10">
              {(metrics?.recentPayouts ?? []).map((payout: any) => (
                <div key={payout.id} className="p-5 flex items-center justify-between hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-colors">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-mono text-sm font-bold text-black dark:text-white uppercase">{payout.id}</p>
                      <p className="text-xs text-black/50 dark:text-white/50">{new Date(payout.date).toLocaleString()}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-6">
                    <span className="font-serif text-xl text-black dark:text-white">${payout.amount.toLocaleString()}</span>
                    <span className={`px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-full ${
                      payout.status === 'paid' ? 'bg-green-500/10 text-green-600' : 'bg-amber-500/10 text-amber-600'
                    }`}>
                      {payout.status}
                    </span>
                  </div>
