    ...
    <div className="overflow-hidden rounded-2xl border border-slate-800/80 bg-slate-900/60 shadow-sm">
      <div className="flex items-center justify-between border-b border-slate-800/80 px-6 py-5">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-base font-bold text-white">Recent Assessment Applications</h3>
        ...
        <table className="w-full text-left text-sm">
          <thead className="bg-slate-950/50 text-xs uppercase tracking-wider text-slate-400">
            <tr>
              ...
              <th className="px-6 py-3 font-semibold">Micro-Internship Assessment</th>
          ...
          <tbody className="divide-y divide-slate-800/60">
            {recentApplications.map((item) => (
              <tr key={item.id} className="transition-colors hover:bg-slate-900/80">
                <td className="px-6 py-4">
                  <p className="font-semibold text-white">{item.role}</p>
                  <p className="text-xs text-slate-400">{item.company}</p>
                </td>
                <td className="px-6 py-4">
                  <p className="font-medium text-slate-200">{item.assessmentTitle}</p>
                ...
                <td className="px-6 py-4 text-xs text-slate-400">{item.submittedAt}</td>
