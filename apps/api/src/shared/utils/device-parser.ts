...
export function parseDeviceFromRequest(req: Request): ParsedDeviceInfo {
  ...
  const city =
    (req.headers["cf-ipcity"] as string) || (req.headers["x-vercel-ip-city"] as string) || null;
  const country =
  ...
  if (
    /ipad|tablet|(android(?!.*mobile))|(windows(?!.*phone)(.*touch))|kindle|silk/i.test(userAgent)
  ) {
    deviceType = "tablet";
  } else if (
    /mobile|iphone|ipod|android.*mobile|blackberry|phone|iemobile|opera mini/i.test(userAgent)
  ) {
    deviceType = "mobile";
