...
export class OpenAIProvider implements IAIProvider {
  ...
  async complete(request: AICompletionRequest): Promise<AICompletionResponse> {
    if (!this.isConfigured()) {
      throw new AIProviderError(AIProvider.OPENAI, "OpenAI API key not configured", false);
    }
    ...
    try {
      ...
      if (choice === undefined) {
        throw new AIProviderError(AIProvider.OPENAI, "No completion choices returned", true);
      }
      ...
      log.warn({ err: error }, "OpenAI API error");
      throw new AIProviderError(
        AIProvider.OPENAI,
