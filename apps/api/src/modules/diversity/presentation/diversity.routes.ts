...
function registerDiversityDeps(): void {
  ...
  try { container.get('DiversityService'); } catch {
    container.register('DiversityService', (infra: InfrastructureDependencies) =>
      new DiversityAnalyticsService(infra.db),
    );
    container.register('DiversityController', () =>
...
export function createDiversityRoutes(): Router {
  ...
  router.get('/company-report', authMiddleware, requireAnyRole([Role.COMPANY_OWNER, Role.ADMIN, Role.SUPER_ADMIN]), ctrl.getCompanyReport);
  router.get('/platform-report', authMiddleware, requireAnyRole([Role.SUPER_ADMIN, Role.ADMIN]), ctrl.getPlatformReport);
