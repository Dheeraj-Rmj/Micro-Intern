...
export class ManagementLoginUseCase {
  ...
  async execute(dto: LoginDto, ipAddress: string) {
    ...
    if (user.isLocked()) {
      ...
      throw new AuthDomainError('AUTH_ACCOUNT_LOCKED', 'Account is temporarily locked due to too many failed login attempts');
    }
    ...
    log.info({ userId: user.id, role: user.role, ipAddress }, 'Management user logged in successfully');
