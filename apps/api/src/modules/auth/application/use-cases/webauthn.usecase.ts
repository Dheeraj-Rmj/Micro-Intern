...
const expectedOrigin =
  config.NODE_ENV === "production"
    ? ["https://ops.microintern.com", "https://micro-intern.vercel.app"]
  ...
  async execute(userId: string, body: any) {
    ...
    if (!expectedChallenge) {
      throw new ValidationError(
        "No active WebAuthn challenge found for user. Please restart registration.",
      );
    }
    ...
    throw new AuthDomainError(
      "AUTH_MFA_TOKEN_INVALID",
      "Verification failed to produce credential info.",
    );
  }
}
  ...
  async execute(userId: string) {
    ...
    if (!userWebAuthnCredentials || userWebAuthnCredentials.length === 0) {
      throw new UnauthorizedError(
        "No WebAuthn credentials found for user. Please use TOTP or register a key.",
      );
    }
