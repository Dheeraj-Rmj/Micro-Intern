...
import { NotificationNotFoundError, UnauthorizedNotificationAccessError } from '../../domain/errors/notification.errors.js';
...
export class MarkNotificationReadUseCase {
  ...
  async execute(userId: string, notificationId: string): Promise<Record<string, unknown>> {
    ...
    if (notification.userId !== userId) {
      log.warn({ notificationId, requestingUserId: userId, ownerId: notification.userId }, 'Unauthorized attempt to modify notification');
