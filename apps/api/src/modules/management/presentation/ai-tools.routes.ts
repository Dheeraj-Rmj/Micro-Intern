...
function registerAIToolsDeps(): void {
  ...
  try { container.get('AIToolsController'); } catch {
    container.register('JobDescriptionService', (infra: InfrastructureDependencies) =>
      new JobDescriptionService(infra.db, infra.aiEngine),
    );
    container.register('GenerateOfferLetterUseCase', (infra: InfrastructureDependencies) =>
      new GenerateOfferLetterUseCase(infra.aiEngine),
    );
    container.register('AIToolsController', () =>
