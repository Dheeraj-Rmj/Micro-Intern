...
import { BcryptPasswordService, RedisSessionService } from '@/modules/auth/infrastructure/services/AuthServices.js';
...
export function createManagementRouter(): Router {
  ...
  try {
    ...
    container.register('ManagementLoginUseCase', () => new ManagementLoginUseCase(
      container.get('IUserRepository'),
    ...
    container.register('InviteRecruiterUseCase', () => new InviteRecruiterUseCase(
      container.get('IUserRepository'),
      container.get('IEmailAuthService'),
      container.get('TokenService'),
    ));

    container.register('InviteAdminUseCase', () => new InviteAdminUseCase(
      container.get('IUserRepository'),
      container.get('IEmailAuthService'),
      container.get('TokenService'),
    ));

    container.register('AcceptInvitationUseCase', () => new AcceptInvitationUseCase(
      container.get('IUserRepository'),
      container.get('IPasswordService'),
      container.get('IJwtService'),
      container.get('ISessionService'),
    ...
    container.register('ManagementController', () => new ManagementController(
  ...
  router.post(
    ...
    (req, res, next) => { controller.acceptInvitation(req, res, next).catch(next); },
  );
  ...
  router.post(
    ...
    (req, res, next) => { controller.inviteRecruiter(req, res, next).catch(next); },
  );
  ...
  router.post(
    ...
    audit(AuditAction.CREATE, 'Invitation'),
    (req, res, next) => { controller.inviteAdmin(req, res, next).catch(next); },
  );
