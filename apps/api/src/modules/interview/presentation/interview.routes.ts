...
function registerInterviewDeps(): void {
  ...
  try { container.get('InterviewService'); } catch {
    container.register('InterviewService', (infra: InfrastructureDependencies) =>
      new InterviewService(infra.db, infra.aiEngine),
    );
    container.register('InterviewController', () =>
...
export function createInterviewRoutes(): Router {
  ...
  router.post('/', authMiddleware, requireAnyRole([Role.RECRUITER, Role.COMPANY_OWNER, Role.ADMIN, Role.SUPER_ADMIN]), ctrl.createInterview);
  router.put('/:id/publish', authMiddleware, requireAnyRole([Role.RECRUITER, Role.COMPANY_OWNER, Role.ADMIN, Role.SUPER_ADMIN]), ctrl.publishInterview);
  router.get('/:id', authMiddleware, ctrl.getInterview);
  router.get('/', authMiddleware, ctrl.listCompanyInterviews);
  router.post('/:id/invite', authMiddleware, requireAnyRole([Role.RECRUITER, Role.COMPANY_OWNER, Role.ADMIN, Role.SUPER_ADMIN]), ctrl.inviteCandidate);
