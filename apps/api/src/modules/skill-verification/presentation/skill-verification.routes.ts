...
export function createSkillVerificationRoutes(): Router {
  ...
  router.post('/verify', authMiddleware, requireAnyRole([Role.ADMIN, Role.SUPER_ADMIN, Role.COMPANY_OWNER, Role.RECRUITER]), controller.verifySkill);
  router.get('/candidate/:candidateId', authMiddleware, controller.getCandidateVerifiedSkills);
  router.get('/candidate/:candidateId/skill/:skillId', authMiddleware, controller.getSkillVerification);
