...
export function createRoleProfileRoutes(): Router {
  ...
  router.post(
    "/:id/skills",
    authMiddleware,
    requireAnyRole([Role.COMPANY_OWNER, Role.RECRUITER, Role.SUPER_ADMIN, Role.ADMIN]),
    controller.addRequiredSkill,
  );
  router.post(
    "/:id/competencies",
    authMiddleware,
    requireAnyRole([Role.COMPANY_OWNER, Role.RECRUITER, Role.SUPER_ADMIN, Role.ADMIN]),
    controller.addRequiredCompetency,
  );
  router.post(
    "/:id/evaluate",
    authMiddleware,
