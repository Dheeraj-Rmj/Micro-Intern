...
export class PrismaAdminRepository implements IAdminRepository {
  ...
  async listTrials(filters: { search?: string; status?: string }): Promise<any[]> {
    ...
    return submissions.map((s) => {
      ...
      else if (s.status === 'UNDER_EVALUATION' || s.status === 'SUBMITTED') escrowStatus = 'REVIEW_PENDING';
  ...
  async getEscrowMetrics(): Promise<any> {
    ...
    const activeCount = await this.db.assessment.count({ where: { status: 'PUBLISHED' as any, deletedAt: null } });
    const pendingCount = await this.db.submission.count({ where: { status: 'SUBMITTED' as any, deletedAt: null } });
  ...
  async getSubscriptionMetrics(): Promise<any> {
    const activeCompanies = await this.db.company.count({ where: { status: 'ACTIVE' as any, deletedAt: null } });
