  ...
  public async execute(roleProfileId: string, companyId: string): Promise<void> {
    ...
    for (const pool of pools) {
      for (const tpCandidate of pool.candidates) {
        try {
          const matchResult = await this.matchingService.matchCandidateToRole(roleProfileId, tpCandidate.candidateId);
          
          if (matchResult.recommendation === 'STRONG_MATCH' || matchResult.recommendation === 'POTENTIAL_MATCH') {
            matchesFound++;
...
`;
            ...
            log.info({ candidateId: tpCandidate.candidateId, score: matchResult.overallMatchScore }, 'Talent pool candidate invited to apply.');
          }
        } catch (error) {
          log.warn({ candidateId: tpCandidate.candidateId, err: error }, 'Failed to evaluate talent pool candidate.');
        }
      }
    }
