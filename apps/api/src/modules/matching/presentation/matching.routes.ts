...
export function createMatchingRoutes(): Router {
  ...
  router.post('/candidate', authMiddleware, requireAnyRole([Role.ADMIN, Role.SUPER_ADMIN, Role.COMPANY_OWNER, Role.RECRUITER]), controller.matchCandidate);
  router.post('/rank', authMiddleware, requireAnyRole([Role.ADMIN, Role.SUPER_ADMIN, Role.COMPANY_OWNER, Role.RECRUITER]), controller.rankCandidates);
