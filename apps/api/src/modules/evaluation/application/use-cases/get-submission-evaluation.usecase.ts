...
import {
  UnauthorizedEvaluationAccessError,
...
export class GetSubmissionEvaluationUseCase {
  ...
  async execute(submissionId: string, requestingUserId: string): Promise<Evaluation> {
    ...
    if (profile && profile.id !== submission.candidateId) {
      ...
      log.warn(
        { submissionId, requestingUserId },
        "Unauthorized attempt to inspect private evaluation",
      );
      throw new UnauthorizedError(
        "Not authorized to access evaluation results for this submission",
        "FORBIDDEN",
      );
    }
