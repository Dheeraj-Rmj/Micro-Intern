...
export function initAIEvaluationWorker(): Worker<AIEvaluationJobData> {
  ...
  log.info({ queue: QUEUE_NAMES.AI_EVALUATION }, 'Initializing BullMQ AI Evaluation Background Worker');

  workerInstance = createWorker<AIEvaluationJobData>(
    QUEUE_NAMES.AI_EVALUATION,
    async (job) => {
      log.info({ jobId: job.id, submissionId: job.data.submissionId }, 'Processing AI evaluation job from BullMQ');
      const container = getContainer();
      const processEvaluationUseCase = container.get<ProcessEvaluationUseCase>('ProcessEvaluationUseCase');
