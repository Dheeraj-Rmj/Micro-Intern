...
import { TrialExpiredError, SubmissionAlreadyCompletedError } from '../errors/submission.errors.js';
  ...
  validateCanSubmit(now: Date = new Date()): void {
    ...
    if (this.expiresAt !== null) {
      ...
      if (now.getTime() > this.expiresAt.getTime() + gracePeriodMs) {
        throw new TrialExpiredError(this.id, this.expiresAt.toISOString());
      }
    }
  }
