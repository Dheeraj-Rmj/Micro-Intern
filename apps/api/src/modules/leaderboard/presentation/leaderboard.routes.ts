...
function registerLeaderboardDeps(): void {
  ...
  try { container.get('LeaderboardService'); } catch {
    container.register('LeaderboardService', (infra: InfrastructureDependencies) =>
      new CandidateLeaderboardService(infra.db),
    );
