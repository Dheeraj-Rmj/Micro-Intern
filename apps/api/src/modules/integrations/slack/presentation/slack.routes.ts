...
function registerSlackDeps(): void {
  ...
  try {
    container.get("SlackService");
  } catch {
    container.register(
      "SlackService",
      (infra: InfrastructureDependencies) => new SlackService(infra.db),
    );
    container.register("SlackController", () => new SlackController(container.get("SlackService")));
  }
}
