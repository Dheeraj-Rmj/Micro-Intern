...
const SLACK_EVENT_TEMPLATES: Record<string, (payload: any) => object> = {
  CANDIDATE_APPLIED: (p) => ({
    blocks: [
      { type: 'header', text: { type: 'plain_text', text: '🎯 New Application Received', emoji: true } },
      { type: 'section', fields: [
        { type: 'mrkdwn', text: `*Candidate:*\n${p.candidateName}` },
        { type: 'mrkdwn', text: `*Role:*\n${p.roleName}` },
      ]},
      { type: 'section', text: { type: 'mrkdwn', text: `*Skill Match:* ${p.skillMatch ?? 'Calculating...'}%` } },
      { type: 'divider' },
    ],
  }),
  ASSESSMENT_SUBMITTED: (p) => ({
    blocks: [
      { type: 'header', text: { type: 'plain_text', text: '📝 Assessment Submitted', emoji: true } },
      { type: 'section', fields: [
        { type: 'mrkdwn', text: `*Candidate:*\n${p.candidateName}` },
  ...
  CANDIDATE_REJECTED: (p) => ({
    blocks: [
      { type: 'header', text: { type: 'plain_text', text: '❌ Application Status Updated', emoji: true } },
      { type: 'section', text: { type: 'mrkdwn', text: `Candidate *${p.candidateName}* was not selected for *${p.roleName}*.` } },
    ],
  }),
  INTERVIEW_SUBMITTED: (p) => ({
    blocks: [
      { type: 'header', text: { type: 'plain_text', text: '🤖 AI Interview Submitted', emoji: true } },
      { type: 'section', fields: [
        { type: 'mrkdwn', text: `*Candidate:*\n${p.candidateName}` },
...
export class SlackService {
  ...
  async configureIntegration(companyId: string, webhookUrl: string, channelName: string, events: string[]) {
