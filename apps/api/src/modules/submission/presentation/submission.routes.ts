...
import { AssessmentParamSchema, PaginationQuerySchema, SubmitAssessmentBodySchema } from '../../evaluation/presentation/evaluation.schemas.js';
...
export function registerSubmissionModuleDependencies(): void {
  ...
  try {
    ...
    container.register('StartAssessmentUseCase', () => new StartAssessmentUseCase(
      container.get('ISubmissionRepository'),
      container.get('IAssessmentRepository'),
      container.get('GetProfileUseCase')
    ));

    container.register('SubmitAssessmentUseCase', () => new SubmitAssessmentUseCase(
      container.get('ISubmissionRepository'),
      container.get('GetProfileUseCase')
    ));
    ...
    container.register('SubmissionController', () => new SubmissionController(
      container.get('StartAssessmentUseCase'),
...
export function attachAssessmentSubmissionRoutes(assessmentRouter: Router): void {
  ...
  assessmentRouter.post(
    ...
    (req, res, next) => { controller.startAssessment(req, res, next).catch(next); }
  );
  ...
  assessmentRouter.post(
    ...
    (req, res, next) => { controller.submitAssessment(req, res, next).catch(next); }
  );
}
