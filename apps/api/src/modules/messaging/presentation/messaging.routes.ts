...
function registerMessagingDeps(): void {
  ...
  try { container.get('MessagingService'); } catch {
    container.register('MessagingService', (infra: InfrastructureDependencies) =>
      new MessagingService(infra.db),
    );
    container.register('MessagingController', () =>
