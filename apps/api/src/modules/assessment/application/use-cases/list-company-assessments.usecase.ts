...
export class ListCompanyAssessmentsUseCase {
  ...
  async execute(
    ...
    log.info(
      { userId, page, limit, status: query.status },
      "Listing company assessment assessments",
    );
    ...
    const { assessments, total } = await this.assessmentRepository.listCompanyAssessments(
      company.id,
      {
        status: query.status,
        ...paginationInput,
