...
export class CandidateJourneyController {
  ...
  listCandidateJourneys = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
  ...
  listCompanyJourneys = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const list = await this.journeyService.listCompanyJourneys(req.params['companyId'] as string, status);
      res.status(200).json({ success: true, data: list });
    } catch (err) {
      next(err);
    }
