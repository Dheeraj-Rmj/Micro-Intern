...
export function registerCandidateJourneyModuleDependencies(): void {
  ...
  try {
    ...
    container.register('CandidateJourneyAutomationListener', (infra: InfrastructureDependencies) => {
      ...
      const verificationService = new SkillVerificationService(new PrismaSkillVerificationRepository(infra.db));
      const listener = new CandidateJourneyAutomationListener(journeyRepo, journeyService, evidenceService, verificationService);
...
export function createCandidateJourneyRoutes(): Router {
  ...
  router.post('/', authMiddleware, requireAnyRole([Role.ADMIN, Role.SUPER_ADMIN, Role.COMPANY_OWNER, Role.RECRUITER]), controller.startJourney);
  router.put('/:id/advance', authMiddleware, requireAnyRole([Role.ADMIN, Role.SUPER_ADMIN, Role.COMPANY_OWNER, Role.RECRUITER]), controller.advanceJourney);
