...
export type GenerateHiringRecommendationOutput = {
  ...
  riskScore: number;
  recommendedAction: 'Reject' | 'Hold' | 'Assign Learning' | 'Technical Interview' | 'HR Interview' | 'Manager Round' | 'Offer' | 'Hire';
  reasoning: string;
};
...
export class GenerateHiringRecommendationUseCase {
  ...
  async execute(input: GenerateHiringRecommendationInput): Promise<GenerateHiringRecommendationOutput> {
