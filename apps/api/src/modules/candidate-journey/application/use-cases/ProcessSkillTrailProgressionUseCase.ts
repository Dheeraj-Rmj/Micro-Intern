...
export type ProcessSkillTrailProgressionInput = {
  journeyId: string;
  performanceClassification: 'Exceptional' | 'Outstanding' | 'Excellent' | 'Above Average' | 'Good' | 'Average' | 'Below Average' | 'Needs Improvement';
  actorId: string;
};
...
export class ProcessSkillTrailProgressionUseCase {
  ...
  async execute(input: ProcessSkillTrailProgressionInput): Promise<void> {
    log.info({ journeyId: input.journeyId, performance: input.performanceClassification }, 'Processing automated Skill Trail progression');
    ...
    try {
      ...
      if (!journey || !journey.roleProfile?.skillTrail?.configuration) {
        log.warn('No Skill Trail Configuration found for this journey, skipping automated progression');
        return;
      }

      const configRules = journey.roleProfile.skillTrail.configuration.rulesJson as unknown as SkillTrailConfigurationType;

      // 2. Determine Action based on performance classification vs required performance
      const levels = ['Needs Improvement', 'Below Average', 'Average', 'Good', 'Above Average', 'Excellent', 'Outstanding', 'Exceptional'];
      ...
      if (candidateLevelIdx >= requiredLevelIdx) {
        ...
        if (configRules.candidateFeedback.enableLearningPlan) {
          log.info('Candidate Feedback is enabled in Config. The Recovery Engine will pick this up.');
