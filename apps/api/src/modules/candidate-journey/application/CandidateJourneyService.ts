...
export class CandidateJourneyService {
  private static readonly VALID_TRANSITIONS: Record<CandidateJourneyStatus, CandidateJourneyStatus[]> = {
    ...
    ASSESSMENT_SUBMITTED: [CandidateJourneyStatus.AI_EVALUATED, CandidateJourneyStatus.HUMAN_REVIEW, CandidateJourneyStatus.REJECTED],
    AI_EVALUATED: [CandidateJourneyStatus.HUMAN_REVIEW, CandidateJourneyStatus.SKILL_VERIFIED, CandidateJourneyStatus.REJECTED],
    HUMAN_REVIEW: [CandidateJourneyStatus.SKILL_VERIFIED, CandidateJourneyStatus.REJECTED],
    SKILL_VERIFIED: [CandidateJourneyStatus.INTERVIEW, CandidateJourneyStatus.OFFER, CandidateJourneyStatus.REJECTED],
    INTERVIEW: [CandidateJourneyStatus.OFFER, CandidateJourneyStatus.SKILL_VERIFIED, CandidateJourneyStatus.REJECTED],
  ...
  async startJourney(data: CreateCandidateJourneyDTO, actorId: string): Promise<CandidateJourney> {
    const existing = await this.journeyRepo.findByCandidateAndCompany(data.candidateId, data.companyId);
    if (existing) {
      return existing;
    }
  ...
  async listCompanyJourneys(companyId: string, status?: CandidateJourneyStatus): Promise<CandidateJourney[]> {
