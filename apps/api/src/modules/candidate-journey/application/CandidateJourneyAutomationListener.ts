...
import { CandidateJourneyStatus, EvidenceType, EvidenceVerificationStatus, SkillVerificationStatus } from '@microintern/database';
...
export class CandidateJourneyAutomationListener {
  ...
  registerListeners(): void {
    log.info('Registering event-driven automation listeners for CandidateJourney, Evidence, and SkillVerification');

    eventBus.on(DOMAIN_EVENTS.ASSESSMENT_SUBMITTED, async (payload: any) => {
      try {
        log.info({ submissionId: payload.submissionId }, 'Automation: Processing ASSESSMENT_SUBMITTED event');
        ...
        await this.evidenceService.registerEvidence(
          {
            ...
            url: payload.repoUrl || `https://app.microintern.com/submissions/${payload.submissionId}`,
        ...
        const activeJourney = journeys.find(
          (j) => j.status === CandidateJourneyStatus.ASSESSMENT_STARTED || j.assessmentId === payload.assessmentId
        );
    ...
    eventBus.on(DOMAIN_EVENTS.EVALUATION_COMPLETED, async (payload: any) => {
      try {
        log.info({ evaluationId: payload.evaluationId }, 'Automation: Processing EVALUATION_COMPLETED event');

        // 1. Mark evidence as verified if percentageScore >= 70
        const submissionEvidence = await this.evidenceService.listSubmissionEvidence(payload.submissionId);
        for (const ev of submissionEvidence) {
          const newStatus = payload.isPassed ? EvidenceVerificationStatus.VERIFIED : EvidenceVerificationStatus.NEEDS_REVIEW;
