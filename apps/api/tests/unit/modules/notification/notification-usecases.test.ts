...
} from "@/modules/notification/domain/errors/notification.errors.js";
...
describe("Notification Use Cases", () => {
  ...
  beforeEach(() => {
    mockRepo = {
      ...
      listByUser: vi
        .fn()
        .mockResolvedValue({ items: [], total: 0, page: 1, limit: 10, totalPages: 1 }),
      update: vi.fn(),
  ...
  describe("MarkNotificationReadUseCase", () => {
    it("should throw NotificationNotFoundError if notification does not exist in repo", async () => {
      ...
      await expect(markReadUseCase.execute("usr-123", "notif-999")).rejects.toThrow(
        NotificationNotFoundError,
      );
    });
    ...
    it("should throw UnauthorizedNotificationAccessError if user is not the notification owner", async () => {
      ...
      await expect(markReadUseCase.execute("usr-stranger", notif.id)).rejects.toThrow(
        UnauthorizedNotificationAccessError,
      );
    });
