...
describe('StartAssessmentUseCase & SubmitAssessmentUseCase', () => {
  ...
  beforeEach(() => {
    ...
    startUseCase = new StartAssessmentUseCase(mockSubRepo, mockAssessmentRepo, mockGetProfileUseCase);
    submitUseCase = new SubmitAssessmentUseCase(mockSubRepo, mockGetProfileUseCase, mockStorageService);
  ...
  it('should throw MaxAttemptsExceededError if candidate already reached max attempts on StartAssessmentUseCase', async () => {
    ...
    await expect(startUseCase.execute('user-1', 'assessment-1')).rejects.toThrow(MaxAttemptsExceededError);
  });
  ...
  it('should start a session, compute expiresAt based on durationMinutes, and emit ASSESSMENT_STARTED', async () => {
    ...
    expect(eventBus.emit).toHaveBeenCalledWith(DOMAIN_EVENTS.ASSESSMENT_STARTED, expect.any(Object));
  });
  ...
  it('should upload solution files to PRIVATE storage, save answers, transition to SUBMITTED, and enqueue BullMQ job', async () => {
    ...
    expect(eventBus.emit).toHaveBeenCalledWith(DOMAIN_EVENTS.ASSESSMENT_SUBMITTED, expect.any(Object));
  });
});
