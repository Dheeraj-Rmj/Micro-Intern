...
describe("Submission & Evaluation Query Use Cases", () => {
  ...
  beforeEach(() => {
    ...
    getEvaluationUseCase = new GetSubmissionEvaluationUseCase(
      mockSubRepo,
      mockEvalRepo,
      mockGetProfileUseCase,
    );
    listSubmissionsUseCase = new ListCandidateSubmissionsUseCase(
      mockSubRepo,
      mockGetProfileUseCase,
    );
  });
  ...
  it("should throw UnauthorizedError if stranger candidate attempts to access private evaluation results", async () => {
    await expect(getEvaluationUseCase.execute("sub-100", "user-stranger")).rejects.toThrow(
      UnauthorizedError,
    );
  });
