...
describe('Candidate Resume Use Cases', () => {
  ...
  beforeEach(() => {
    ...
    mockStorage = {
      upload: vi.fn().mockResolvedValue({ url: 'https://storage.example.com/private/resumes/resume.pdf' }),
      getSignedDownloadUrl: vi.fn().mockResolvedValue({ url: 'https://signed.example.com/resume.pdf?token=123' }),
    };
  ...
  describe('UploadResumeUseCase', () => {
    ...
    it('should reject resume exceeding 5MB max file size limit', async () => {
      const useCase = new UploadResumeUseCase(mockDb, mockStorage, mockCalcCompletion, mockQueue);
      const file: any = { mimetype: 'application/pdf', size: 6 * 1024 * 1024, buffer: Buffer.from('data') };
    ...
    it('should upload resume to PRIVATE bucket, update database, and queue job for AI parsing', async () => {
      ...
      const file: any = { mimetype: 'application/pdf', size: 2048, buffer: Buffer.from('pdf content') };
  ...
  describe('GetResumeUrlUseCase', () => {
    ...
    it('should generate a 15-minute signed S3 download URL from PRIVATE bucket', async () => {
      const useCase = new GetResumeUrlUseCase(mockDb, mockStorage);
      mockDb.candidateProfile.findUnique.mockResolvedValue({ resumeUrl: 'resumes/prof-1/resume_v1000.pdf' });
