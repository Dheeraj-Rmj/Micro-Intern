...
import { CompanyNotFoundError, NotCompanyOwnerError, InvalidFileTypeError, FileTooLargeError } from '@/modules/company/domain/errors/company.errors.js';
...
describe('Company Management & Branding Use Cases', () => {
  ...
  beforeEach(() => {
    ...
    mockStorage = {
      upload: vi.fn().mockResolvedValue({ url: 'https://storage.example.com/logos/comp-1/logo_v1000.webp' }),
    };
  });
  ...
  describe('UpdateCompanyUseCase', () => {
    it('should throw NotCompanyOwnerError when a non-owner attempts to update company configuration', async () => {
      ...
      await expect(useCase.execute('user-recruiter', { name: 'Hacked Name' } as any)).rejects.toThrow(NotCompanyOwnerError);
