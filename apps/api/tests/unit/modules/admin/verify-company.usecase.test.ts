...
import { VerifyCompanyUseCase, ListPendingCompaniesUseCase } from '@/modules/admin/application/index.js';
import { AdminCompanyNotFoundError, CompanyAlreadyVerifiedError } from '@/modules/admin/domain/index.js';
...
describe('VerifyCompany & ListPendingCompanies Use Cases', () => {
  ...
  describe('VerifyCompanyUseCase', () => {
    it('should throw AdminCompanyNotFoundError if company ID does not exist', async () => {
      ...
      await expect(verifyUseCase.execute('admin-1', 'comp-not-found')).rejects.toThrow(AdminCompanyNotFoundError);
    });
    ...
    it('should throw CompanyAlreadyVerifiedError if company is already ACTIVE', async () => {
      ...
      await expect(verifyUseCase.execute('admin-1', 'comp-100')).rejects.toThrow(CompanyAlreadyVerifiedError);
    });
    ...
    it('should update company status to ACTIVE and emit COMPANY_VERIFIED domain event', async () => {
      ...
      expect(mockRepository.updateCompanyStatus).toHaveBeenCalledWith('comp-100', EntityStatus.ACTIVE);
  ...
  describe('ListPendingCompaniesUseCase', () => {
    it('should list all companies waiting for verification in repository', async () => {
      const pending = [
        { id: 'comp-1', name: 'A', slug: 'a', status: EntityStatus.PENDING_VERIFICATION, createdAt: new Date() },
      ];
