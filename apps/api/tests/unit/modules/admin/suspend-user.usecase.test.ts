...
import { AdminUserNotFoundError, UserAlreadySuspendedError, CannotSuspendAdminError } from '@/modules/admin/domain/index.js';
...
describe('SuspendUserUseCase', () => {
  ...
  it('should throw AdminUserNotFoundError if user ID does not exist', async () => {
    ...
    await expect(useCase.execute('admin-1', 'user-not-found')).rejects.toThrow(AdminUserNotFoundError);
  });
  ...
  it('should throw UserAlreadySuspendedError if user is already suspended', async () => {
    ...
    await expect(useCase.execute('admin-1', 'bad-actor')).rejects.toThrow(UserAlreadySuspendedError);
    expect(mockSessionService.revokeAllSessions).not.toHaveBeenCalled();
  });
  ...
  it('should update user status to SUSPENDED, invoke revokeAllSessions in Redis immediately, and emit USER_SUSPENDED', async () => {
    ...
    expect(mockRepository.updateUserStatus).toHaveBeenCalledWith('bad-actor', EntityStatus.SUSPENDED);
